...
with f_col1:
    ...
    d["in_daskroi"] = st.number_input("223.670 NMC HR (Daskroi) Demand (cfs)", value=d["in_daskroi"], min_value=0.0, step=5.0)
    ...
    d["in_sab_esc"] = st.number_input("229.340 Sabarmati Escape (cfs)", value=d["in_sab_esc"], min_value=0.0, step=5.0)
...
with f_col2:
    ...
    d["in_dholka"] = st.number_input("246.447 HR (Dholka Branch) (cfs)", value=d["in_dholka"], min_value=0.0, step=5.0)
...
with f_col3:
    ...
    d["in_east_esc"] = st.number_input("255.742 Eastern Drain Escape (cfs)", value=d["in_east_esc"], min_value=0.0, step=5.0)
    d["in_piyaj"] = st.number_input("256.220 Piyaj Dharoi Pipeline (cfs)", value=d["in_piyaj"], min_value=0.0, step=5.0)
    d["in_sanand"] = st.number_input("258.849 HR (Sanand Branch) (cfs)", value=d["in_sanand"], min_value=0.0, step=5.0)
    d["in_saurashtra"] = st.number_input("263.309 HR (Saurashtra Branch) (+/- cfs)", value=d["in_saurashtra"], min_value=None, step=10.0)
    d["in_karannagar"] = st.number_input("263.574 CR (Karannagar CR) (+/- cfs)", value=d["in_karannagar"], min_value=None, step=10.0)
